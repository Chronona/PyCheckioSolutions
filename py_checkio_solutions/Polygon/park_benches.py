...
def count_bench_ren(benches, dist):
    ...
    for i in range(0, len(benches)-1):
        if benches[i][1] + dist > benches[i+1][1]:
            print("ng")
        else:
            print("ok")
            length += benches[i][1] - benches[i][0]
    length += benches[-1][1] - benches[-1][0]
    return length


def park_benches(benches: List[Tuple[int, int]], dist: int) -> int:
    benches = sorted([sorted(i) for i in a ])

    print("result", count_bench_ren(benches, dist))
    
    benches = [list(map(lambda x: -1 * x, j)) for j in sorted([sorted(i, reverse=True) for i in a], reverse=True)]
    print("result", count_bench_ren(benches, dist))
...
# %%
a = [(1, 3), (6, 5), (13, 4)]
[list(map(lambda x: -1 * x, j)) for j in sorted([sorted(i, reverse=True) for i in a], reverse=True)]
# %%
[j for j in a ]
# %%
